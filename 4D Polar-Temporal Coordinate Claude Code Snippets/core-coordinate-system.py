...
if __name__ == "__main__":
    # Create the 4D space
    space = PolarTemporalSpace(context_layers=3)
    
    # Create some points
    p1 = PolarTemporalCoordinate(r=1.0, theta=0.5, t=100, z=1)
    p2 = PolarTemporalCoordinate(r=2.0, theta=1.5, t=105, z=1)
    p3 = PolarTemporalCoordinate(r=1.5, theta=0.6, t=102, z=2)
    
    # Calculate distances
    d12 = space.distance(p1, p2)
    d13 = space.distance(p1, p3)
    
    print(f"Distance p1→p2: {d12:.2f}")
    print(f"Distance p1→p3: {d13:.2f}")
    
    # Navigate from a point
    p4 = space.navigate(p1, delta_r=0.5, delta_theta=0.1, delta_t=1, delta_z=1)
    print(f"After navigation: {p4}")
    
    # Find nearest neighbors
    points = [p1, p2, p3, p4]
    neighbors = space.nearest_neighbors(p1, points, k=2)
    print("Nearest neighbors to p1:")
