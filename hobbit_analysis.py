"""
Temporal Analysis of The Hobbit with results saved to file
"""
...
# Patterns for temporal expressions in literary text
...
class TemporalMatch:
    ...
    def to_dict(self):
        return {
            ...
            "position": f"{self.start}-{self.end}"
        }

def extract_literary_temporal_expressions(text: str):
...
def analyze_hobbit_excerpts():
    """Analyze all excerpts and return structured results."""
    all_results = []
    total_matches = 0
    ...
    return {
        ...
        "excerpt_count": len(HOBBIT_EXCERPTS),
        "total_temporal_expressions": total_matches,
        "results": all_results
    }

def main():
    """Main function to run the analysis and save results."""
    print("Analyzing temporal expressions in excerpts from 'The Hobbit'...")
    
    # Run the analysis
    analysis_results = analyze_hobbit_excerpts()
    
    # Save to file
    output_file = "hobbit_temporal_analysis_results.json"
    with open(output_file, 'w') as f:
        json.dump(analysis_results, f, indent=2)
    
    # Print summary
    print(f"Analysis complete!")
    print(f"Found {analysis_results['total_temporal_expressions']} temporal expressions in {analysis_results['excerpt_count']} excerpts.")
    print(f"Results saved to: {output_file}")
    
    # Also print a few examples
    if analysis_results['total_temporal_expressions'] > 0:
        print("\nSample of matches found:")
        count = 0
        for result in analysis_results['results']:
            if result['matches']:
                for match in result['matches'][:2]:  # Show up to 2 matches per excerpt
                    print(f" - {match['text']} ({match['pattern_type']})")
                    count += 1
                    if count >= 5:  # Show up to 5 matches total
                        break
            if count >= 5:
                break
